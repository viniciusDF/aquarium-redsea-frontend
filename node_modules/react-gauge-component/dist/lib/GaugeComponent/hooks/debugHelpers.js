 ...
 */
export var drawDebugOverlay = function (gauge, layout) {
    var debugGroup = gauge.g.current.append('g').attr('class', 'debug-overlay');
    ...
    debugGroup.append('text')
        ...
        .attr('fill', 'blue')
        .attr('font-size', '10px')
        .text("outer: ".concat(layout.outerRadius.toFixed(1)));
    debugGroup.append('text')
        .attr('x', 5)
        .attr('y', -layout.innerRadius + 15)
        .attr('fill', 'green')
        .attr('font-size', '10px')
        .text("inner: ".concat(layout.innerRadius.toFixed(1)));
};
/**
 * Removes the debug overlay
 */
export var clearDebugOverlay = function (gauge) {
 ...
 */
export var drawViewBoxBounds = function (gauge, layout) {
    var viewBox = layout.viewBox;
 ...
 */
export var clearViewBoxBounds = function (gauge) {
    gauge.svg.current.selectAll('.debug-viewbox').remove();
};
/**
 * Logs detailed layout information to console
 */
export var logLayoutInfo = function (gauge, layout, parentWidth, parentHeight) {
 ...
 */
export var createRenderCounter = function (threshold) {
    ...
    return function () {
        renderCount++;
        var now = Date.now();
        var elapsed = now - lastResetTime;
        // Reset counter every second
        if (elapsed > 1000) {
            if (renderCount > threshold) {
                console.warn("\u26A0\uFE0F High render count: ".concat(renderCount, " renders in ").concat(elapsed, "ms"));
 ...
 */
export var validateLayout = function (layout, parentWidth, parentHeight) {
    var errors = [];
    // Check for NaN values
    if (isNaN(layout.outerRadius))
        errors.push('outerRadius is NaN');
    if (isNaN(layout.innerRadius))
        errors.push('innerRadius is NaN');
    if (isNaN(layout.viewBox.width))
        errors.push('viewBox.width is NaN');
    if (isNaN(layout.viewBox.height))
        errors.push('viewBox.height is NaN');
    // Check for invalid values
    if (layout.outerRadius <= 0)
        errors.push('outerRadius must be positive');
    if (layout.innerRadius < 0)
        errors.push('innerRadius must be non-negative');
    if (layout.innerRadius > layout.outerRadius)
        errors.push('innerRadius cannot exceed outerRadius');
    if (layout.viewBox.width <= 0)
        errors.push('viewBox.width must be positive');
    if (layout.viewBox.height <= 0)
        errors.push('viewBox.height must be positive');
    // Check for unreasonable values
    if (layout.outerRadius > parentWidth * 2) {
        errors.push("outerRadius (".concat(layout.outerRadius, ") is unreasonably large for parent width (").concat(parentWidth, ")"));
    }
    if (layout.outerRadius > parentHeight * 2) {
        errors.push("outerRadius (".concat(layout.outerRadius, ") is unreasonably large for parent height (").concat(parentHeight, ")"));
    }
    if (errors.length > 0) {
        console.error('❌ Invalid Layout Detected:');
        errors.forEach(function (err) { return console.error('  -', err); });
 ...
 */
export var drawTestPattern = function (gauge) {
    var testGroup = gauge.g.current.append('g').attr('class', 'test-pattern');
    var _a = gauge.dimensions.current, outerRadius = _a.outerRadius, innerRadius = _a.innerRadius;
    // Draw angle markers every 45 degrees
    var angles = [0, 45, 90, 135, 180, 225, 270, 315];
    angles.forEach(function (degrees) {
        var radians = (degrees - 90) * Math.PI / 180; // -90 to start from top
        var x = outerRadius * Math.cos(radians);
        var y = outerRadius * Math.sin(radians);
    ...
    // Draw radius markers
    var radii = [innerRadius, (innerRadius + outerRadius) / 2, outerRadius];
    var radiusLabels = ['inner', 'mid', 'outer'];
 ...
 */
export var clearTestPattern = function (gauge) {
    gauge.g.current.selectAll('.test-pattern').remove();
};
/**
 * Comprehensive debug mode - enables all visualizations
 */
export var enableDebugMode = function (gauge, layout, parentWidth, parentHeight) {
 ...
 */
export var disableDebugMode = function (gauge) {
